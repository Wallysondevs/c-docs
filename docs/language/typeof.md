...
`typeof` e `typeof_unqual` são coletivamente chamados de _operadores typeof_. Os operadores `typeof` não podem ser aplicados a membros de bit-field. Se o tipo do operando for um tipo modificado variavelmente, o operando é avaliado; caso contrário, o operando não é avaliado. O resultado do operador `typeof_unqual` é o tipo não atômico não qualificado que resultaria do operador `typeof`. O operador `typeof` preserva todos os qualificadores.
