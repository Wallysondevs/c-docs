...
Verifica se o caractere fornecido possui uma representação gráfica, ou seja, se é um número (`0123456789`), uma letra maiúscula (`ABCDEFGHIJKLMNOPQRSTUVWXYZ`), uma letra minúscula (`abcdefghijklmnopqrstuvwxyz`), ou um caractere de pontuação (`!"#$%&'()*+,-./:;<=>?@[\]^_`{|}~`), ou qualquer caractere gráfico específico da localidade C atual.

O comportamento é indefinido se o valor de `ch` não puder ser representado como unsigned char e não for igual a [EOF](<#/doc/io>).
...
Valor diferente de zero se o caractere tiver uma representação gráfica, zero caso contrário.
...
### Referências
  ...
  * 7.4.1.6 A função isgraph (p: 146)

  * Padrão C11 (ISO/IEC 9899:2011):

    

  * 7.4.1.6 A função isgraph (p: 201-202)

  * Padrão C99 (ISO/IEC 9899:1999):

    

  * 7.4.1.6 A função isgraph (p: 182-183)

  * Padrão C89/C90 (ISO/IEC 9899:1990):

    

  * 4.3.1.5 A função isgraph

### Veja também

[ iswgraph](<#/doc/string/wide/iswgraph>)(C95) | verifica se um caractere largo é um caractere gráfico
(função)
[Documentação C++](<#/>) para isgraph
...
[`iswblank`](<#/doc/string/wide/iswblank>) | `isgraph`
