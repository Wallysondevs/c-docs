...
```c
int vwprintf( const wchar_t* format, va_list vlist );  // desde C95
(até C99)  // até C99
int vwprintf( const wchar_t* restrict format, va_list vlist );  // desde C99
int vfwprintf( FILE* stream, const wchar_t* format, va_list vlist );  // desde C95
(até C99)  // até C99
int vfwprintf( FILE* restrict stream,
const wchar_t* restrict format, va_list vlist );  // desde C99
int vswprintf( wchar_t* buffer, size_t bufsz,
const wchar_t* format, va_list vlist );  // desde C95
(até C99)  // até C99
int vswprintf( wchar_t* restrict buffer, size_t bufsz,
const wchar_t* restrict format, va_list vlist );  // desde C99
int vwprintf_s( const wchar_t* restrict format, va_list vlist);  // desde C11
int vfwprintf_s( FILE* restrict stream,
const wchar_t* restrict format, va_list vlist);  // desde C11
...
3) Escreve os resultados para um buffer de wide string. No máximo bufsz - 1 wide characters são escritos, seguidos por um caractere wide nulo. A wide string resultante será terminada com um caractere wide nulo, a menos que bufsz seja zero.

4-6) O mesmo que (1-3), exceto que os seguintes erros são detectados em tempo de execução e chamam a função [manipuladora de restrição](<#/doc/error/set_constraint_handler_s>) atualmente instalada: 
  ...
  * (apenas para `vswprintf_s`), a string a ser armazenada em buffer (incluindo o wide nulo final) excederia bufsz.

7) O mesmo que (6), exceto que truncará o resultado para caber dentro do array apontado por buffer. 

    Assim como todas as funções com verificação de limites, `vwprintf_s`, `vfwprintf_s`, `vswprintf_s` e `vsnwprintf_s` são garantidas de estarem disponíveis apenas se __STDC_LIB_EXT1__ for definido pela implementação e se o usuário definir __STDC_WANT_LIB_EXT1__ para a constante inteira 1 antes de incluir [`<stdio.h>`](<#/doc/io>).

### Parâmetros

stream  |  \-  |  fluxo wide de saída para escrever   
buffer  |  \-  |  ponteiro para uma wide string para escrever   
bufsz  |  \-  |  número máximo de wide characters a serem escritos   
format  |  \-  |  ponteiro para uma wide string terminada em nulo especificando como interpretar os dados   
vlist  |  \-  |  [lista de argumentos variáveis](<#/doc/language/variadic>) contendo os dados a serem impressos.   
  
  
A string de **format** consiste em wide characters comuns (exceto `%`), que são copiados inalterados para o fluxo de saída, e especificações de conversão. Cada especificação de conversão tem o seguinte formato: 
  ...
  * (opcional) valor inteiro ou `*` que especifica a largura mínima do campo. O resultado é preenchido com caracteres de _espaço_ (por padrão), se necessário, à esquerda quando justificado à direita, ou à direita quando justificado à esquerda. No caso em que `*` é usado, a largura é especificada por um argumento adicional do tipo int, que aparece antes do argumento a ser convertido e do argumento que fornece a precisão, se um for fornecido. Se o valor do argumento for negativo, resulta na flag `-` especificada e largura de campo positiva (Nota: Esta é a largura mínima: O valor nunca é truncado.). 

    

  * (opcional) `.` seguido por um número inteiro ou `*`, ou nenhum dos dois, que especifica a _precisão_ da conversão. No caso em que `*` é usado, a _precisão_ é especificada por um argumento adicional do tipo int, que aparece antes do argumento a ser convertido, mas depois do argumento que fornece a largura mínima do campo, se um for fornecido. Se o valor deste argumento for negativo, ele é ignorado. Se nem um número nem `*` for usado, a precisão é considerada zero. Veja a tabela abaixo para os efeitos exatos da _precisão_. 
...
Não-um-número é convertido para `nan` ou `nan(_sequência_de_caracteres_)`. Qual delas é usada é definida pela implementação. 
...
1,4) O número de wide characters escritos se bem-sucedido ou valor negativo se ocorreu um erro.

3) O número de wide characters escritos se bem-sucedido ou valor negativo se ocorreu um erro. Se a string resultante for truncada devido ao limite de bufsz, a função retorna o número total de caracteres (não incluindo o caractere wide nulo de terminação) que teriam sido escritos, se o limite não tivesse sido imposto.

2,5) número de wide characters transmitidos para o fluxo de saída ou valor negativo se ocorreu um erro de saída, um erro de violação de restrições em tempo de execução ou um erro de codificação.

6) número de wide characters escritos no buffer, sem contar o caractere wide nulo (que é sempre escrito desde que buffer não seja um ponteiro nulo e bufsz não seja zero e não seja maior que `RSIZE_MAX/sizeof(wchar_t)`), ou zero em violações de restrição em tempo de execução, e valor negativo em erros de codificação.

7) número de wide characters não incluindo o caractere nulo de terminação (que é sempre escrito desde que buffer não seja um ponteiro nulo e bufsz não seja zero e não seja maior que `RSIZE_MAX/sizeof(wchar_t)`), que teriam sido escritos no buffer se bufsz fosse ignorado, ou um valor negativo se ocorreu uma violação de restrições em tempo de execução ou um erro de codificação.
...
Enquanto strings estreitas fornecem [vsnprintf](<#/doc/io/vfprintf>), o que torna possível determinar o tamanho do buffer de saída necessário, não há equivalente para wide strings (até `vsnwprintf_s` do C11), e para determinar o tamanho do buffer, o programa pode precisar chamar `vswprintf`, verificar o valor do resultado e realocar um buffer maior, tentando novamente até ter sucesso. 

`vsnwprintf_s`, ao contrário de `vswprintf_s`, truncará o resultado para caber dentro do array apontado por buffer, mesmo que o truncamento seja tratado como um erro pela maioria das funções com verificação de limites. 
...
(função)  
[ wprintffwprintfswprintfwprintf_sfwprintf_sswprintf_ssnwprintf_s](<#/doc/io/fwprintf>)(C95)(C95)(C95)(C11)(C11)(C11)(C11) |  imprime saída formatada de wide characters para [stdout](<#/doc/io/std_streams>), um fluxo de arquivo ou um buffer   
(função)  
