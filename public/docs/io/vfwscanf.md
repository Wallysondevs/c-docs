...
```c
int vwscanf( const wchar_t *restrict format, va_list vlist );  // desde C99
int vfwscanf( FILE *restrict stream,
const wchar_t *restrict format, va_list vlist );  // desde C99
int vswscanf( const wchar_t *restrict buffer,
const wchar_t *restrict format, va_list vlist );  // desde C99
int vwscanf_s( const wchar_t *restrict format, va_list vlist );  // desde C11
...
Lê dados de uma variedade de fontes, interpreta-os de acordo com `format` e armazena os resultados em locais definidos por `vlist`.
...
Assim como todas as funções com verificação de limites, `vwscanf_s`, `vfwscanf_s` e `vswscanf_s` são garantidas de estarem disponíveis apenas se __STDC_LIB_EXT1__ for definido pela implementação e se o usuário definir __STDC_WANT_LIB_EXT1__ para a constante inteira 1 antes de incluir [`<stdio.h>`](<#/doc/io>).
...
A string **format** consiste em
  ...
  * caracteres de espaço em branco: qualquer caractere de espaço em branco único na string de formato consome todos os caracteres de espaço em branco consecutivos disponíveis da entrada (determinado como se chamando [`iswspace`](<#/doc/string/wide/iswspace>) em um loop). Observe que não há diferença entre "\n", " ", "\t\t" ou outros espaços em branco na string de formato.
  ...
  * (opcional) número inteiro (maior que zero) que especifica a _largura máxima do campo_, ou seja, o número máximo de caracteres que a função pode consumir ao fazer a conversão especificada pela especificação de conversão atual. Observe que %s e %[ podem levar a estouro de buffer se a largura não for fornecida.
...
Todas essas funções podem invocar va_arg, o valor de `arg` é indeterminado após o retorno. Essas funções não invocam va_end, e isso deve ser feito pelo chamador.
...
### Referências
  ...
  * 7.29.2.6 A função vfwscanf (p: 418)

  * 7.29.2.8 A função vswscanf (p: 419)

  * 7.29.2.10 A função vwscanf (p: 420)

  * K.3.9.1.7 A função vfwscanf_s (p: 632-633)

  * K.3.9.1.10 A função vswscanf_s (p: 635-636)

  * K.3.9.1.12 A função vwscanf_s (p: 637)

  * Padrão C99 (ISO/IEC 9899:1999):

  * 7.24.2.6 A função vfwscanf (p: 364)

  * 7.24.2.8 A função vswscanf (p: 365)

  * 7.24.2.10 A função vwscanf (p: 366)
...
(função)
[Documentação C++](<#/>) para vwscanf, vfwscanf, vswscanf