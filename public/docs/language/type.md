...
| (desde C99)  
  ...
  * tipos de ponto flutuante reais: float, double, long double

    

    

  * tipos de ponto flutuante decimais reais: _Decimal32, _Decimal64, _Decimal128
...
#### Grupos de tipos
  ...
  * _tipos reais_ : tipos inteiros e tipos de ponto flutuante reais
...
Os tipos `T` e `U` são compatíveis, se
  ...
  * eles são versões cvr-qualificadas identicamente de tipos compatíveis não qualificados
  ...
  * ambos são tipos struct/union/enumeração, e
  ...
  * adicionalmente, se forem structs ou unions,

    

  * Os membros correspondentes devem ser declarados na mesma ordem (apenas structs)
  ...
  * um é uma definição de estilo antigo (sem parâmetros), o outro tem uma lista de parâmetros, a lista de parâmetros não usa reticências e cada parâmetro é compatível (após o ajuste do tipo do parâmetro da função) com o parâmetro de estilo antigo correspondente após as promoções de argumento padrão
  * um é uma declaração de estilo antigo (sem parâmetros), o outro tem uma lista de parâmetros, a lista de parâmetros não usa reticências, e todos os parâmetros (após o ajuste do tipo do parâmetro da função) não são afetados pelas promoções de argumento padrão
...
Um tipo composto pode ser construído a partir de dois tipos que são compatíveis; é um tipo que é compatível com ambos os dois tipos e satisfaz as seguintes condições:
  ...
  * Se ambos os tipos forem tipos array, as seguintes regras são aplicadas:

    

  * Se um tipo for um array de tamanho constante conhecido, o tipo composto é um array desse tamanho.

    

  * Caso contrário, se um tipo for um VLA cujo tamanho é especificado por uma expressão que não é avaliada, um programa que necessita do tipo composto de ambos os tipos tem comportamento indefinido.
  * Caso contrário, se um tipo for um VLA cujo tamanho é especificado, o tipo composto é um VLA desse tamanho.
  * Caso contrário, se um tipo for um VLA de tamanho não especificado, o tipo composto é um VLA de tamanho não especificado.
...
| (desde C99)  
  ...
  * Se apenas um tipo for um tipo função com uma lista de tipos de parâmetros (um protótipo de função), o tipo composto é um protótipo de função com a lista de tipos de parâmetros.

| (até C23)  
  
  * Se ambos os tipos forem tipos função com listas de tipos de parâmetros, o tipo de cada parâmetro na lista de tipos de parâmetros composta é o tipo composto dos parâmetros correspondentes.
...
Nomes de tipo são usados nas seguintes situações:
